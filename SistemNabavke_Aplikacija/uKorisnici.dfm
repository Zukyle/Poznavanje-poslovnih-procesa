object fraKorisnici: TfraKorisnici
  Left = 0
  Top = 0
  Width = 980
  Height = 700
  TabOrder = 0
  object pnlHeader: TPanel
    Left = 0
    Top = 0
    Width = 980
    Height = 90
    Align = alTop
    BevelOuter = bvNone
    Color = 16513270
    ParentBackground = False
    TabOrder = 0
    object lblNaslov: TLabel
      Left = 24
      Top = 18
      Width = 80
      Height = 30
      Caption = 'Korisnici'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -21
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblOpis: TLabel
      Left = 24
      Top = 52
      Width = 166
      Height = 15
      Caption = 'Upravljanje korisnicima sistema'
    end
  end
  object pnlDetalji: TPanel
    AlignWithMargins = True
    Left = 660
    Top = 102
    Width = 300
    Height = 586
    Margins.Left = 12
    Margins.Top = 12
    Margins.Right = 20
    Margins.Bottom = 12
    Align = alRight
    BevelOuter = bvNone
    Color = clWhite
    ParentBackground = False
    TabOrder = 1
    object lblDetaljiNaslov: TLabel
      Left = 16
      Top = 18
      Width = 144
      Height = 17
      Caption = 'Podaci o korisniku'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'JetBrains Mono SemiBold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblUsernameNaslov: TLabel
      Left = 16
      Top = 64
      Width = 78
      Height = 15
      Caption = 'Korisni'#269'ko ime'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGray
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object lblImeNaslov: TLabel
      Left = 16
      Top = 120
      Width = 71
      Height = 30
      Caption = 'Ime i prezime'#10
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGray
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object lblUlogaNaslov: TLabel
      Left = 16
      Top = 176
      Width = 31
      Height = 15
      Caption = 'Uloga'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGray
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object lblLozinkaNaslov: TLabel
      Left = 16
      Top = 232
      Width = 40
      Height = 15
      Caption = 'Lozinka'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clGray
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
    end
    object edtUsername: TEdit
      Left = 16
      Top = 85
      Width = 268
      Height = 23
      TabOrder = 0
    end
    object edtIme: TEdit
      Left = 16
      Top = 141
      Width = 268
      Height = 23
      TabOrder = 1
    end
    object edtLozinka: TEdit
      Left = 16
      Top = 253
      Width = 268
      Height = 23
      PasswordChar = '#'
      TabOrder = 2
    end
    object cmbUloga: TComboBox
      Left = 16
      Top = 197
      Width = 268
      Height = 23
      Style = csDropDownList
      TabOrder = 3
    end
    object chkAktivan: TCheckBox
      Left = 16
      Top = 296
      Width = 97
      Height = 17
      Caption = 'Aktivan'
      Checked = True
      State = cbChecked
      TabOrder = 4
    end
    object btnNovi: TButton
      Left = 16
      Top = 483
      Width = 265
      Height = 33
      Caption = 'Novi korisnik'
      TabOrder = 5
    end
    object Panel1: TPanel
      Left = 15
      Top = 530
      Width = 268
      Height = 31
      BevelOuter = bvNone
      Color = clWhite
      ParentBackground = False
      TabOrder = 6
      object btnSacuvaj: TButton
        Left = 0
        Top = 0
        Width = 126
        Height = 31
        Align = alLeft
        Caption = 'Sa'#269'uvaj'
        TabOrder = 0
        ExplicitTop = 16
      end
      object btnOdustani: TButton
        Left = 142
        Top = 0
        Width = 126
        Height = 31
        Align = alRight
        Caption = 'Odustani'
        TabOrder = 1
      end
    end
  end
  object pnlLista: TPanel
    AlignWithMargins = True
    Left = 12
    Top = 102
    Width = 636
    Height = 586
    Margins.Left = 12
    Margins.Top = 12
    Margins.Right = 0
    Margins.Bottom = 12
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    ParentBackground = False
    TabOrder = 2
    object lblListaNaslov: TLabel
      Left = 20
      Top = 16
      Width = 135
      Height = 19
      Caption = 'Lista korisnika'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'JetBrains Mono'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblPretraga: TLabel
      Left = 308
      Top = 18
      Width = 80
      Height = 17
      Caption = 'Pretraga :'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'JetBrains Mono'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object dbgKorisnici: TDBGrid
      AlignWithMargins = True
      Left = 20
      Top = 58
      Width = 596
      Height = 508
      Margins.Left = 20
      Margins.Top = 20
      Margins.Right = 20
      Margins.Bottom = 20
      Align = alBottom
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -12
      TitleFont.Name = 'Segoe UI'
      TitleFont.Style = []
    end
    object edtPretraga: TEdit
      Left = 394
      Top = 16
      Width = 222
      Height = 23
      TabOrder = 1
    end
  end
end
